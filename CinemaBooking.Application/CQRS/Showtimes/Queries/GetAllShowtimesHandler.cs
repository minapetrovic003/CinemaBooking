...
public class GetAllShowtimesHandler : IRequestHandler<GetAllShowtimesQuery, IEnumerable<ShowtimeDto>>
{
    ...
    public GetAllShowtimesHandler(IUnitOfWork uow) => _uow = uow;

    public Task<IEnumerable<ShowtimeDto>> Handle(
        GetAllShowtimesQuery request, CancellationToken cancellationToken)
    {
        var result = _uow.Showtimes
            .Search(request.MovieTitle, request.FromDate)
            .Select(MapToDto);

        return Task.FromResult<IEnumerable<ShowtimeDto>>(result);
    }
